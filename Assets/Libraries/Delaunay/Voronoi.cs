...
namespace Delaunay
{
    public sealed class Voronoi : Utils.IDisposable
    {
        ...
        public Voronoi(int pointCount, Rect plotBounds)
        {
            ...
            for (int i = 0; i < pointCount; i++)
            {
                ...
                _points.Add(new Vector2(
                        UnityEngine.Random.Range(0, plotBounds.width),
                        UnityEngine.Random.Range(0, plotBounds.height))
                );
            }

            Init(_points, colors, plotBounds);
        }

        public Voronoi(int pointCount, Rect plotBounds, int lloydIterations)
        {
            List<uint> colors = new List<uint>();
            _points = new List<Vector2>();
        ...
        public void LloydRelaxation(int nbIterations)
        {
            ...
            for (int i = 0; i < nbIterations; i++)
            {
                ...
                _points.Clear();
                _points = newPoints;
                Init(newPoints, _colors, origPlotBounds);
            }
        }
